# @mini-dev/location-wx

跨微信 / 支付宝 / 抖音小程序设备定位库的**微信（wx）平台包**。
...
- **方法级增强**：`getLocation` / `getFuzzyLocation` 各为自带拦截器链 + 缓存的可调用函数
...
```bash
npm i @mini-dev/location-wx
```

> **微信开发者工具**需额外执行 **工具 → 构建 npm**，把 `@mini-dev/location-wx` 及其传递依赖 `@mini-dev/location`、`koa-compose` 一起构建进 `miniprogram_npm`，否则运行时会报找不到 `koa-compose`。

## 快速开始

从平台包导入现成单例（首次调用才绑定全局 `wx`，import 不会抛错）：
...
> ⚠️ 缓存命中会在 `cache` 层短路，**其后的权限 / 自定义拦截器不会执行**。含 `permission` 的方法若同时开 `maxAge`，命中缓存时权限检查会被跳过（含用户已撤销授权的窗口期仍会返回旧位置）。若权限必须每次校验，请**对该方法不启用 `maxAge`**，或为带权限校验的调用单独建一个不带缓存的 `createMethod` 实例。

> ⚠️ `AbortSignal` / `AbortController` 由消费方保证运行时可用。部分老引擎未原生提供，需自行 polyfill 或确认目标平台支持后再使用 `ext.signal`。库不内置 polyfill。

## 权限闭环（可选拦截器）

`permission` 是可选中间件（不进默认链）：`getSetting → 未授权则 authorize → blocked 抛错`。弹窗 / 跳 `wx.openSetting` 的 UX 交给调用方。本包的 `permission` 已注入微信默认 scope。
...
`authSetting[scope]` 三态：`true` 放行；`false` 视为 blocked；`undefined` 未问过，调 `authorize` 弹窗，失败抛 `PERMISSION_DENIED`。默认 scope：`getLocation` → `scope.userLocation`，`getFuzzyLocation` → `scope.userFuzzyLocation`；可通过 `opts.scope` 显式覆盖。

> 默认单例已内置缓存 / 超时 / 重试 / 取消（经 `ext` 激活）。只有需要 `permission` 权限闭环或**方法级**自定义拦截器时，才用 `createMethod` 自建实例；`permission` 不能直接挂到默认单例上。
...
- **原生错误**：**原样 reject**（微信 `{errMsg}`），库不翻译、不合并、不打 `code`
...
### `getLocation(opts?)`
默认单例，等价 `createMethod('getLocation', { provider: createWxProvider })`。返回 `Promise<wx.getLocation 结果>` 或回调模式下 `undefined`。

### `getFuzzyLocation(opts?)`
默认单例，模糊定位。返回 `Promise<wx.getFuzzyLocation 结果>` 或 `undefined`。

### `createWxProvider(): Provider`
构造 wx Provider：首次调用懒绑全局 `wx`，注入 wx 的 `isTransientError`。

### `permission(opts?)`
权限中间件包装，注入微信默认 scope。`opts`：`scope?`（显式覆盖默认）、`onBlocked?(e: LocationError)`。

### 再导出（来自 `@mini-dev/location`）
`createMethod` · `createProvider` · `LocationError` · 类型 `Provider` · `isTransientError`（wx 瞬态策略）。

## 平台说明

- `type` 坐标系：`'wgs84'`（默认）/ `'gcj02'`。
- 微信要求 `getFuzzyLocation` 与 `getLocation` 在 `requiredPrivateInfos` 里互斥，一个 app 只能二选一。
- 本包仅在微信环境使用，不防御 `wx` 全局缺席（属 misuse）。

## 相关

- 仓库：<https://github.com/xesam/minidev-location>
- 核心引擎：[`@mini-dev/location`](https://www.npmjs.com/package/@mini-dev/location)
- 同族平台包：[`@mini-dev/location-my`](https://www.npmjs.com/package/@mini-dev/location-my) · [`@mini-dev/location-tt`](https://www.npmjs.com/package/@mini-dev/location-tt)

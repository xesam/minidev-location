...
describe('wx permission 包装', () => {
    ...
    it('getFuzzyLocation 默认 scope.userFuzzyLocation', async () => {
        const authorize = jest.fn((o: any) => o.success());
        const p = createProvider(nativeWith({ authSetting: {}, authorize }), isTransientError);
        const next = jest.fn(async () => {});
    ...
    it('显式 scope 覆盖默认', async () => {
        ...
        const p = createProvider(nativeWith({ authSetting: {}, authorize }), isTransientError);
        const next = jest.fn(async () => {});
        await permission({ scope: 'scope.userLocationBackground' })(ctx(p, 'getFuzzyLocation'), next);

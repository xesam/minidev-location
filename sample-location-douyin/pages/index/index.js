// @mini-dev/location 抖音小程序示例（v1 方法级 API）
//
// 本示例演示库的全部特性，按分栏组织：
...
//
// 平台由独立包选定（@mini-dev/location-tt），不探测；import 不会抛错。
// 抖音仅支持 gcj02 坐标系、且只有 getLocation（无 getFuzzyLocation / watchLocation）。
// 平台包再导出核心 API（createMethod / permission / LocationError），消费方只装一个平台包即可。
import { getLocation, createMethod, permission, LocationError, createTtProvider } from '@mini-dev/location-tt';
...
// 抛普通 Error 且无 errMsg → 抖音策略 isTransientError 判为瞬态 → runWithRetry 会重跑整条链。
...
const cachedGetLocation = createMethod('getLocation', { provider: createTtProvider, cacheSize: 5 });
...
Page({
    ...
    // ===== 1. 基础使用 =====
    // 抖音 type 仅支持 'gcj02'（坐标系），原样透传；库控制放 ext。
